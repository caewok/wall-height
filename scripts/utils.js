...
export function getWallBounds(wall) {
    ...
    const top = wall.document.flags[MODULE_SCOPE]?.[TOP_KEY] ?? Infinity;
    const bottom = wall.document.flags[MODULE_SCOPE]?.[BOTTOM_KEY] ?? -Infinity;
    return { top, bottom }
}

export function getLevelsBounds(document){
    const top = document.document.flags?.levels?.rangeTop ?? Infinity;
    const bottom = document.document.flags?.levels?.rangeBottom ?? -Infinity;
    return { top, bottom }
}

export function getSceneSettings(scene) {
    let advancedVision = scene.document.flags[MODULE_SCOPE]?.advancedVision ?? true;
